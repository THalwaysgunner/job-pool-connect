...
const JobDetailPage: React.FC<{ role: "client" | "provider" | "admin" }> = ({ role }) => {
  ...
  useEffect(() => { if (activeTab === "messages") setSeenCount("messages", messages.length); }, [messages.length]);
  useEffect(() => { if (activeTab === "questions") setSeenCount("questions", questions.length); }, [questions.length]);
  useEffect(() => { if (activeTab === "payments") setSeenCount("payments", paymentRequests.length); }, [paymentRequests.length]);
  ...
  const navItems = [
    { key: "summary", label: t("job.summary"), icon: Info, badge: 0 },
    { key: "messages", label: t("job.messages"), icon: MessageSquare, badge: unreadMessages },
    { key: "questions", label: t("job.qa"), icon: HelpCircle, badge: unreadQuestions },
    { key: "payments", label: t("job.payments"), icon: CreditCard, badge: unreadPayments },
    { key: "deliverables", label: t("job.delivery"), icon: Package, badge: 0 },
    ...(role === "client" ? [{ key: "disputes", label: t("job.disputes"), icon: FileWarning, badge: 0 }] : []),
  ];
  ...
  useEffect(() => { return () => { outletContext?.setPageTitle?.(""); }; }, []);

  if (!job) return <div className="p-4 text-muted-foreground">{t("job.loading")}</div>;
  ...
  return (
    <div className="flex h-[calc(100vh-theme(spacing.14))] -m-6">
          ...
          <div className="flex flex-col h-full">
              ...
              <div className="space-y-0">
                {(() => {
                  ...
                  return topLevel.map((m, idx) => {
                    ...
                    const dateStr = isToday ? t("job.today") : isYesterday ? t("job.yesterday") : msgDate.toLocaleDateString();
                    ...
                    return (
                      <React.Fragment key={m.id}>
                        ...
                        <MessageItem message={m} replies={msgReplies} reactions={msgReactions} senderProfiles={senderProfiles} isLocked={isLocked} jobId={id!} onRefresh={fetchAll} allMessages={messages} />
                      </React.Fragment>
                    );
                  });
                })()}
            ...
            {!isLocked && (
              <div className="border-t p-4 flex gap-2 shrink-0 bg-background">
                <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={t("job.typeMessage")} onKeyDown={(e) => e.key === "Enter" && sendMessage()} className="flex-1" />
        ...
        {activeTab === "summary" && (
          <ScrollArea className="flex-1 p-6">
            <div className="space-y-3 text-sm text-start">
              ...
              <Separator />
              <p className="font-medium">{t("job.documents")} ({jobDocs.length})</p>
              {jobDocs.map((d) => (
              ...
              {role === "client" && job.status === "waiting_for_client_approval" && !isLocked && (
                <Button onClick={markDone}><Check className="h-4 w-4 me-2" />{t("job.markDone")}</Button>
              )}
            ...
            <div className="space-y-4">
              {questions.map((q) => (
                <div key={q.id} className="border rounded-lg p-3">
                  ...
                  <Badge variant={q.status === "answered" ? "default" : "secondary"} className="mt-1">{translateStatus(q.status)}</Badge>
                  {q.answer && <p className="text-sm mt-2">A: {q.answer}</p>}
                  {role === "client" && q.status === "open" && !isLocked && (
                    <div className="flex gap-2 mt-2">
                      <Input value={newAnswer[q.id] || ""} onChange={(e) => setNewAnswer((p) => ({ ...p, [q.id]: e.target.value }))} placeholder={t("job.yourAnswer")} />
                      <Button size="sm" onClick={() => answerQuestion(q.id)}>{t("job.answer")}</Button>
                    </div>
                  )}
                </div>
              ))}
              {role === "provider" && !isLocked && (
                <div className="flex gap-2">
                  <Input value={newQuestion} onChange={(e) => setNewQuestion(e.target.value)} placeholder={t("job.askClient")} />
                  <Button onClick={askQuestion}>{t("job.ask")}</Button>
                </div>
              )}
            </div>
          </ScrollArea>
                ...
                <div key={pr.id} className="border rounded-lg p-3">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-end">
                      ...
                      <Badge variant={pr.status === "paid_confirmed_by_client" ? "default" : "secondary"}>{translateStatus(pr.status)}</Badge>
                    </div>
                  </div>
                  {role === "client" && pr.status === "sent" && !isLocked && (
                    <Button size="sm" className="mt-2" onClick={() => confirmPayment(pr.id)}>{t("job.markAsPaid")}</Button>
                  )}
                </div>
              ))}
              {role === "provider" && !isLocked && (
                <div className="space-y-2 border-t pt-4">
                  <Input placeholder={t("job.title")} value={newPR.title} onChange={(e) => setNewPR({ ...newPR, title: e.target.value })} />
                  <Input type="number" placeholder={t("job.amount")} value={newPR.amount} onChange={(e) => setNewPR({ ...newPR, amount: e.target.value })} />
                  <Textarea placeholder={t("job.details")} value={newPR.details} onChange={(e) => setNewPR({ ...newPR, details: e.target.value })} />
                  <Button onClick={createPaymentRequest}>{t("job.createPaymentRequest")}</Button>
                </div>
              )}
            </div>
          </ScrollArea>
            ...
            <div className="space-y-4">
              {disputes.map((d) => (
                <div key={d.id} className="border rounded-lg p-3">
                  ...
                  <Badge variant={d.status === "resolved" ? "default" : "destructive"} className="mt-1">{translateStatus(d.status)}</Badge>
                  {d.resolution && <p className="text-sm mt-2 text-muted-foreground">{t("job.resolution")}: {d.resolution}</p>}
                </div>
              ))}
              {!isLocked && (
                <div className="flex gap-2">
                  <Textarea value={disputeReason} onChange={(e) => setDisputeReason(e.target.value)} placeholder={t("job.describeIssue")} />
                  <Button onClick={openDispute} className="shrink-0"><AlertTriangle className="h-4 w-4 me-1" />{t("job.openDispute")}</Button>
                </div>
              )}
            </div>
          </ScrollArea>
      ...
      <aside className="w-64 border-s bg-sidebar text-sidebar-foreground flex flex-col shrink-0">
          ...
          <div className="grid grid-cols-2 gap-3">
            {navItems.map((item) => (
              ...
              >
                ...
                {item.badge > 0 && (
                  <span className="absolute top-1.5 end-1.5 inline-flex items-center justify-center h-5 min-w-5 rounded-full bg-destructive text-destructive-foreground text-[10px] font-semibold px-1">
                    {item.badge}
                  </span>
                )}
              </button>
...
const DeliverableSection: React.FC<{ role: string; jobId: string; deliverables: any[]; isLocked: boolean; onRefresh: () => void }> = ({ role, jobId, deliverables, isLocked, onRefresh }) => {
  ...
  const types = [
    { value: "website", labelKey: "deliverable.website" },
    { value: "strategy_plan", labelKey: "deliverable.strategyPlan" },
    { value: "ads_plan", labelKey: "deliverable.adsPlan" },
    { value: "invoice", labelKey: "deliverable.invoice" },
  ];
  ...
  return (
    <Card>
      <CardContent className="p-4 space-y-4">
        {types.map((tp) => {
          const items = deliverables.filter((d) => d.deliverable_type === tp.value);
          return (
            <div key={tp.value} className="border rounded-lg p-3">
              <p className="font-medium text-sm mb-2">{t(tp.labelKey)}</p>
              {items.map((item) => (
                <div key={item.id} className="flex items-center justify-between text-sm mb-1">
                  {item.website_url ? (
                    <div>
                      ...
                      {item.website_credentials && <p className="text-muted-foreground">{t("deliverable.credentials")}: {item.website_credentials}</p>}
                    </div>
                  ) : (
                ...
                <>
                  {tp.value === "website" ? (
                    <div className="space-y-2 mt-2">
                      <Input placeholder={t("deliverable.websiteUrl")} value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} />
                      <Input placeholder={t("deliverable.credentials")} value={websiteCreds} onChange={(e) => setWebsiteCreds(e.target.value)} />
                      <Button size="sm" onClick={saveWebsite}>{t("deliverable.save")}</Button>
                    </div>
                  ) : (
                    <label className="cursor-pointer mt-2 inline-block">
                      <Button variant="outline" size="sm" asChild><span><Upload className="h-3 w-3 me-1" />{t("deliverable.upload")}</span></Button>
                      <Input type="file" className="hidden" onChange={(e) => { if (e.target.files?.[0]) uploadDeliverable(tp.value, e.target.files[0]); }} />
                    </label>
                  )}
                </>
              )}
